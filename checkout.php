...
for($i=1; $i < $content['itemCount'] + 1; $i++) {
    ...
    $checkam = mysqli_query($con, "select * from current_stock left join book on current_stock.book_id = book.book_id where current_stock.book_id ='".$split_id[1]."'");
    $check = mysqli_fetch_array($checkam);

    if( (int) $quantity > (int) $check['current_amount'])
    {
      $_SESSION['errorbook'] = $check['title'];
